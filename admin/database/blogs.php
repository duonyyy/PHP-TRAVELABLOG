...
class blogs{
	...
	//Read all records
	public function getNextBlogByUser($currentBlogId, $userId) {
        $query = "SELECT id, title FROM blogs WHERE id_user = ? AND id > ? ORDER BY id ASC LIMIT 1";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(1, $userId);
        $stmt->bindParam(2, $currentBlogId);
        $stmt->execute();
        return $stmt->fetch(PDO::FETCH_ASSOC);
    }
    public function getPreviousBlogByUser($currentBlogId, $userId) {
        $query = "SELECT id, title FROM blogs WHERE id_user = ? AND id < ? ORDER BY id DESC LIMIT 1";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(1, $userId);
        $stmt->bindParam(2, $currentBlogId);
        $stmt->execute();
