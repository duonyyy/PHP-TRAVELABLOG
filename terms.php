...
// Initialize and fetch blog categories
$blogcategories = new blogcategories($db);
$stmt_blogcategories = $blogcategories->readAll();

// Initialize and fetch social links
$socials = new socials($db);
$stmt_socials = $socials->readAll();

// Initialize and fetch links
$links = new links($db);
$stmt_links = $links->readAll();

// Initialize and read terms
$terms = new terms($db);

$terms->id = 1;
$terms->read();
// Initialize and read about information
$about = new about($db);
$about->id = 1;
$about->read();

// Verify subscriber code if present
if (!empty($_GET['verify'])) {
    $subscribers = new subscribers($db);
    $subscribers->verified_token = htmlspecialchars($_GET['verify']);
